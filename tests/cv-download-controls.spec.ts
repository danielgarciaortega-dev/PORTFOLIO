...
for (const localeCase of localeCases) {
  test(
    `${localeCase.locale} CV keeps download in the desktop top-right utility cluster`,
    async ({ page }) => {
      ...
      const geometry = await page.evaluate((downloadLabel) => {
        ...
        const actions =
          document.querySelector<HTMLElement>('.cv-page-actions');
  ...
  for (const width of [360, 390, 430]) {
    test(
      `${localeCase.locale} CV top controls fit at ${width}px without covering document content`,
      async ({ page }) => {
        ...
        const geometry = await page.evaluate((downloadLabel) => {
          ...
          const locale =
            document.querySelector<HTMLElement>('.cv-locale-link');
          const download = Array.from(
            document.querySelectorAll<HTMLAnchorElement>('.download-btn'),
          ).find((node) => node.getAttribute('aria-label') === downloadLabel);
        ...
        expect(
          geometry?.backRight ?? Number.MAX_SAFE_INTEGER,
        ).toBeLessThanOrEqual(geometry?.actionsLeft ?? 0);
        expect(
          geometry?.navBottom ?? Number.MAX_SAFE_INTEGER,
        ).toBeLessThanOrEqual((geometry?.sheetTop ?? 0) + 1);
