...
test('homepage shell and hero use the approved English copy', async ({ page }) => {
  await page.goto('./');

  const mainNavigation = page.getByRole('navigation', {
    name: 'Main navigation',
  });
  await expect(mainNavigation.getByRole('link', { name: 'Home' })).toBeVisible();
  await expect(
  ...
  await expect(footer).toContainText('Full-stack web developer · Granada, Spain');
});
...
test('mobile navigation exposes English accessible names', async ({ page }) => {
  ...
  await expect(menu.getByRole('navigation', { name: 'Mobile navigation' })).toContainText(
    'Home',
  );
