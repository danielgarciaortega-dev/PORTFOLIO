...
test('profile namespaces preserve shared facts and locale-specific copy', () => {
  assert.deepEqual(sharedProfileFields(esProfile), sharedProfileFields(enProfile));
...
test('education and experience namespaces keep facts aligned without copy leakage', () => {
  ...
  assert.equal(esEducation[0].title, 'FP Grado Superior en Desarrollo de Aplicaciones Web');
  assert.equal(enEducation[0].title, 'Higher Technician in Web Application Development');
