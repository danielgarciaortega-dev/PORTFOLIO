...
test(
  'Spanish root renders the restored shell, hero and professional overview',
  async ({ page }) => {
    await page.goto('./');
...
test(
  'language switcher navigates between locale routes and persists explicit choice',
  async ({ page }) => {
    await page.goto('./');
    ...
    await expect(
      spanishSwitcher.locator('[aria-current="true"]'),
    ).toHaveText('ES');
    await expect(
    ...
    await expect(
      englishSwitcher.locator('[aria-current="true"]'),
    ).toHaveText('EN');
