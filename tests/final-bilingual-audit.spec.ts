...
test('all three canonical ES/EN route pairs form one deterministic counterpart system', async ({
  ...
  for (const pair of counterpartPairs) {
    ...
    await expect(page.locator(pair.selector), `${pair.name} ES counterpart`).toHaveCount(1);
    expect(await counterpartPathname(page, pair.selector)).toBe(pair.en.pathname);
    ...
    await expect(page.locator(pair.selector), `${pair.name} EN counterpart`).toHaveCount(1);
    expect(await counterpartPathname(page, pair.selector)).toBe(pair.es.pathname);
...
test('curated public UI landmarks do not mix Spanish and English shell copy', async ({
  ...
  for (const route of ['./', './proyectos/']) {
    ...
    await expect(navigation.getByText('Sobre mí', { exact: true })).toBeVisible();
    await expect(navigation.getByText('Proyectos', { exact: true })).toBeVisible();
    ...
    await expect(navigation.getByText('Projects', { exact: true })).toHaveCount(0);
  }
  ...
  for (const route of ['./en/', './en/projects/']) {
    ...
    await expect(navigation.getByText('Projects', { exact: true })).toBeVisible();
    await expect(navigation.getByText('Inicio', { exact: true })).toHaveCount(0);
    await expect(navigation.getByText('Sobre mí', { exact: true })).toHaveCount(0);
    await expect(navigation.getByText('Proyectos', { exact: true })).toHaveCount(0);
  }

  await page.goto('./cv/');
  await expect(page.locator('.portfolio-back-link')).toContainText('Volver al portfolio');
  await expect(page.locator('.role')).toHaveText('DESARROLLADOR WEB FULL-STACK');
  await expect(page.getByText('Back to portfolio', { exact: true })).toHaveCount(0);

  await page.goto('./en/cv/');
  await expect(page.locator('.portfolio-back-link')).toContainText('Back to portfolio');
  await expect(page.locator('.role')).toHaveText('FULL-STACK WEB DEVELOPER');
  await expect(page.getByText('Volver al portfolio', { exact: true })).toHaveCount(0);
});
...
test('non-canonical English projects alias and representative locale misses remain real 404s', async ({
  ...
  for (const route of ['./projects/', './missing-final-audit/', './en/missing-final-audit/']) {
