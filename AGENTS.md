...
- The active repository ruleset requires pull requests, resolved review conversations and the named repository checks; owner-level pull-request bypass is recovery capability, not permission to ignore repository failures.
...
`Preview readiness` validates exact-head Vercel Preview evidence when the provider is available. If the repository owner explicitly authorizes pull-request recovery because Vercel/provider infrastructure is unavailable, that bypass applies only to the external Preview blocker. It must never be used to conceal a failing `Repository validation`, reuse Preview evidence from another SHA, or promote Vercel as production.
